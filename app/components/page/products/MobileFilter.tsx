...
export function MobileFilter() {
  ...
  // {subCategoryId && '(1)'}
  // {priceRangeId && '(1)'}
  // {clothingGenderId.length > 0 && `(${clothingGenderId.length})`}
  // {colorId.length > 0 && `(${colorId.length})`}
    ...
    <div className="lg:hidden">
      <Drawer>
        <DrawerTrigger asChild>
          <Button className="bg-surface-container relative grid aspect-square h-10 place-items-center rounded-md">
            {changedFiltersCount !== 0 && (
              <div className="bg-tertiary text-on-tertiary absolute -top-3 -right-3 rounded-full px-2.5 py-0.5 text-sm">
                {changedFiltersCount}
              </div>
            )}
            <Funnel size={20} />
        ...
        <DrawerContent className="px-4">
          ...
          <DialogDescription>Narrow down your search by category</DialogDescription>
          <div className="minimalist-scrollbar h-fit overflow-auto pt-4 pb-10">
